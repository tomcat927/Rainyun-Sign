...
def process_captcha(max_retry=5):
    for attempt in range(1, max_retry + 1):
        try:
            logger.info(f"验证码处理尝试 {attempt}/{max_retry}")
    ...
    logger.error("验证码处理失败，已超过最大重试次数")
    return False
...
def compute_similarity(img1_path, img2_path):
    ...
    if len(good) == 0 or len(matches) == 0:
        return 0.0, 0
...
def main():
    global logger, ocr, det, wait, driver, debug, linux
