...
def init_selenium() -> WebDriver:
    ...
    if linux:
        ...
        ops.add_argument("--disable-dev-shm-usage")  # 防止内存不足
        ops.add_argument("--window-size=1920,1080")  # 设置窗口大小
        # 👇 指定使用 chrome-headless-shell 二进制文件
        ops.binary_location = "/usr/local/bin/chrome-headless-shell"

        # 使用您已配置的 chromedriver 路径
...
if __name__ == "__main__":
    ...
    try:
        ...
        send_dingtalk_message("【雨云签到失败】页面加载超时，请检查网络或延长超时时间。")
        exit()
    try:
    ...
    if driver.current_url == "https://app.rainyun.com/dashboard":
        ...
        logger.error("登录失败！")
        send_dingtalk_message("【雨云签到失败】登录失败，请检查账号或密码是否正确。")